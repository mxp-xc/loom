...
interface LocalSkill { id: string; path?: string; targets?: string[] }
interface ManifestData {
  skills: { sources: SkillSource[]; skills: LocalSkill[] }
  config: { targets?: string[] }
  errors: string[]
}
interface ScanMember { name: string; description: string; path: string; installed: boolean }
...
const menuBtnStyle: React.CSSProperties = {
  ...
  background: 'none', cursor: 'pointer', fontFamily: "'Fira Code', monospace", fontSize: 11, color: 'var(--text)',
}
const menuStyle: React.CSSProperties = {
  position: 'absolute', zIndex: 10, border: '1px solid var(--border)', borderRadius: 4,
  background: 'var(--card)', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', minWidth: 120,
}
const refreshBtnStyle: React.CSSProperties = {
  flexShrink: 0, width: 34, fontFamily: "'Fira Code', monospace", fontSize: 15,
  borderRadius: 4, border: '1px solid var(--border)', background: 'var(--bg)', color: 'var(--muted)',
  cursor: 'pointer',
}
const copyBtnStyle: React.CSSProperties = {
  flexShrink: 0, padding: '3px 8px', fontFamily: "'Fira Code', monospace", fontSize: 10,
  borderRadius: 3, border: '1px solid var(--border)', background: 'transparent', color: 'var(--muted)',
  cursor: 'pointer',
}
...
export default function Skills({ repoPath }: { repoPath: string }) {
  ...
  const copyPath = async (p: string) => {
    ...
    try { await navigator.clipboard.writeText(p); setCopiedPath(p); setTimeout(() => setCopiedPath(null), 1500) }
  ...
  const project = async () => {
    ...
    try {
      ...
      if (res.ok) { showToast('投影完成'); load() }
      else { setError(res.message || '投影失败') }
  ...
  const handleCheck = async (src: SkillSource) => {
    ...
    try {
      ...
      if (res.updates?.[0]?.hasUpdate) {
        const latest = res.updates[0].latestTag ?? 'unknown'
 ...
 const handleScan = async () => {
   if (scanning) return
   if (!srcUrl.trim()) { setAddErr('url 不能为空'); return }
   setScanning(true); setAddErr(null); setScanMembers([])
    try {
      const res = await api.scanSource(srcUrl.trim()) as any
      if (Array.isArray(res.members)) {
        setScanMembers(res.members)
        setScanSelected(new Set(res.members.filter((m: ScanMember) => !m.installed).map((m: ScanMember) => m.name)))
      } else {
  ...
  const resetAddForm = () => {
    setAddErr(null); setLocalPath(''); setSrcUrl(''); setSrcRef('main'); setAddTab('local')
    setScanMembers([]); setScanSelected(new Set())
  }
  ...
  const handleAddLocal = async () => {
    ...
    if (!path) { setAddErr('path 不能为空'); return }
    const id = path.replace(/[\\/]+$/, '').split(/[\\/]/).filter(Boolean).pop() ?? ''
    if (!id) { setAddErr('无法从 path 提取 id'); return }
  ...
  const derivedLocalId = localPath.trim()
    ? (localPath.trim().replace(/[\\/]+$/, '').split(/[\\/]/).filter(Boolean).pop() ?? '')
    : ''
    ...
    <div>
      <div className="head">
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
          <button className="add-btn" onClick={openAdd}>+ Add skill</button>
      ...
      {sourceCount === 0 && localCount === 0 && manifest && (
        <div style={{ marginTop: 18, padding: 32, border: '1px solid var(--border)', borderRadius: 6, textAlign: 'center', color: 'var(--muted)' }}>
          ...
          <p style={{ marginTop: 4, fontFamily: "'Fira Code', monospace", fontSize: 12 }}>点击右上 <b>+ Add skill</b> 添加 source 或 local skill</p>
        </div>
      )}
        ...
        return (
          <div key={src.url + '-' + src.ref} className="group">
            <div className="group-head" style={{ position: 'relative' }}>
              ...
              {updates[src.url] && <span style={{ fontFamily: "'Fira Code', monospace", fontSize: 10, color: 'var(--warn)' }}>{'-> '}{updates[src.url]}</span>}
              <span className="gacts">
                <button className="gbtn" onClick={() => handleCheck(src)} disabled={checking === src.url}>
                  {checking === src.url ? '...' : 'check'}
                </button>
                <button className="gbtn" onClick={() => setMenuOpen(menuOpen === src.url ? null : src.url)}>⋯</button>
              </span>
              {menuOpen === src.url && (
                <div style={{ ...menuStyle, right: 14, top: '100%' }}>
                  <button style={menuBtnStyle} onClick={() => { setMenuOpen(null); showToast('Scan: ' + (src.members?.length ?? 0) + ' members') }}>scan</button>
                  <button style={{ ...menuBtnStyle, color: 'var(--error)' }} onClick={() => handleDeleteSource(src.url)}>删除</button>
                </div>
              )}
            </div>
            {src.members?.map((m) => {
              ...
              return (
                <div key={m.name} className="skill">
                  ...
                  <span
                    ...
                    onClick={() => setDetail({ skillId: `${repoId}-${m.name}`, source: src.url, targets: mTargets })}
                  >
                    {repoId}-{m.name}
                  </span>
                  <span className="chips">
                    {allAgents.map(a => renderChip(a, isEnabled && mTargets.includes(a), () => handleChipToggle(src.url, m.name, a, mTargets)))}
                  </span>
            ...
            {!src.members?.length && (
              <div className="skill">
                ...
                <span className="chips">{allAgents.map(a => renderChip(a, true, () => handleChipToggle(src.url, '', a, agents)))}</span>
      ...
      {(manifest?.skills?.skills?.length ?? 0) > 0 && manifest && (
        <div className="group">
          ...
          {manifest.skills.skills.map((s) => {
            ...
            return (
              <div key={s.id} className="skill" style={{ gridTemplateColumns: '12px 1fr auto 90px 28px' }}>
                <span className="sdot green" />
                <span className="sname clickable" onClick={() => setDetail({ skillId: s.id, path: s.path, targets: lTargets })}>{s.id}</span>
                <span className="chips">{allAgents.map(a => renderChip(a, lTargets.includes(a), () => handleLocalChipToggle(s.id, a, lTargets)))}</span>
                <span className="sstate st-proj">projected</span>
                <span style={{ position: 'relative', display: 'flex', justifyContent: 'flex-end' }}>
                  <button className="gbtn" onClick={() => setMenuOpen(menuOpen === 'local:' + s.id ? null : 'local:' + s.id)}>⋯</button>
                  {menuOpen === 'local:' + s.id && (
                    <div style={{ ...menuStyle, right: 0, top: '100%', minWidth: 100 }}>
                      <button style={{ ...menuBtnStyle, color: 'var(--error)' }} onClick={() => handleDeleteLocal(s.id)}>删除</button>
                    </div>
                  )}
                </span>
              </div>
      ...
      <Modal open={addOpen} onClose={closeAdd} title="Add Skill">
        <div style={{ display: 'flex', gap: 4, marginBottom: 16 }}>
          {(['local', 'source'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => { setAddTab(tab); setAddErr(null) }}
              style={{
                flex: 1, padding: '6px 0', fontSize: 12, fontFamily: "'Fira Code', monospace",
                fontWeight: 500, borderRadius: 4, border: '1px solid var(--border)',
        ...
        {addTab === 'local' ? (
          <>
            <div style={{ marginBottom: 14 }}>
              <span className="label">path <span style={{ color: 'var(--error)' }}>*</span></span>
              <input value={localPath} onChange={e => setLocalPath(e.target.value)} placeholder="./assets/skills/my-skill" style={inputStyle} />
              <div style={{ marginTop: 6, fontSize: 11, color: 'var(--muted)' }}>本地 skill 目录路径，id 将从目录名自动提取</div>
              <div style={{ marginTop: 4, fontFamily: "'Fira Code', monospace", fontSize: 11, color: 'var(--signal)' }}>id: {derivedLocalId || '—'}</div>
            </div>
            <button className="add-btn" onClick={handleAddLocal} disabled={addBusy} style={{ width: '100%' }}>
          ...
          <>
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={srcUrl} onChange={e => setSrcUrl(e.target.value)} placeholder="https://github.com/org/repo" style={inputStyle} />
            </div>
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                <input
                  ...
                  onBlur={() => { if (srcUrl.trim()) handleScan() }}
                  placeholder="main"
            ...
            {scanMembers.length > 0 && (
              <div style={{ marginBottom: 14, border: '1px solid var(--border)', borderRadius: 4, maxHeight: 200, overflow: 'auto' }}>
                {scanMembers.map(m => (
                  <label key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderBottom: '1px solid var(--border)', cursor: m.installed ? 'default' : 'pointer', opacity: m.installed ? 0.5 : 1 }}>
                    <input type="checkbox" checked={scanSelected.has(m.name)} disabled={m.installed} onChange={e => {
                      setScanSelected(prev => { const n = new Set(prev); if (e.target.checked) n.add(m.name); else n.delete(m.name); return n })
                    }} />
                    <span style={{ fontFamily: "'Fira Code', monospace", fontSize: 12 }}>{m.name}</span>
                    {m.installed && <span style={{ fontSize: 10, color: 'var(--muted)' }}>(已安装)</span>}
                  </label>
                ))}
              </div>
            )}
            <button className="add-btn" onClick={handleAddSource} disabled={addBusy} style={{ width: '100%' }}>
      ...
      {/* Skill Detail Modal */}
      <Modal open={!!detail} onClose={() => setDetail(null)} title={detail?.skillId ?? ''} width={520}>
        {detail && (
          <div>
            {detail.source && (
              <div style={{ marginBottom: 12 }}>
                <div className="label">source</div>
                <div style={{ marginTop: 4, fontFamily: "'Fira Code', monospace", fontSize: 12, color: 'var(--text)', wordBreak: 'break-all' }}>{detail.source}</div>
              </div>
            )}
            {detail.path && (
              <div style={{ marginBottom: 12 }}>
                <div className="label">path</div>
                <div style={{ marginTop: 4, fontFamily: "'Fira Code', monospace", fontSize: 12, color: 'var(--text)', wordBreak: 'break-all' }}>{detail.path}</div>
              </div>
            )}
            <div style={{ marginBottom: 12 }}>
            ...
            <div>
              ...
              <div style={{ marginTop: 6, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {allAgents.map(a => {
                  ...
                  return (
                    <div key={a} style={{ display: 'flex', alignItems: 'center', gap: 8, opacity: active ? 1 : 0.45 }}>
                      {renderChip(a, active)}
                      <span style={{ flex: 1, fontFamily: "'Fira Code', monospace", fontSize: 11, color: 'var(--text)', wordBreak: 'break-all' }}>{p}</span>
                      <button onClick={() => copyPath(p)} style={copyBtnStyle} title="复制">{copiedPath === p ? 'copied' : 'copy'}</button>
                    </div>
                  )
                })}
              </div>
