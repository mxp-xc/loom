...
function translateError(msg: string): string {
  ...
  if (msg.includes('no such remote') || msg.includes('No configured push destination')) return ERROR_MAP.no_remote
  if (msg.includes('Authentication failed') || msg.includes('401') || msg.includes('403')) return ERROR_MAP.auth_failed
  ...
  if (msg.includes('ENOTFOUND') || msg.includes('ETIMEDOUT') || msg.includes('network')) return ERROR_MAP.network
  return msg
}
...
export default function Sync({ repoPath }: { repoPath: string }) {
 ...
 const [remoteErr, setRemoteErr] = useState<string | null>(null)
  ...
  useEffect(() => {
    api.getSyncRemote(repoPath)
      .then((r) => { setRemoteUrl(r.remoteUrl ?? ''); setRemoteLoaded(true) })
 ...
 const run = async (label: string, fn: () => Promise<unknown>, set: (v: unknown) => void) => {
   ...
   try {
     ...
     if (result && typeof result === 'object' && 'ok' in result && (result as any).ok === false) {
      ...
      set(result)
    } catch (e) { setError(translateError(e instanceof Error ? e.message : String(e))) }
    finally { setBusy(null) }
  }

  const saveRemote = async () => {
    if (!remoteUrl.trim()) { setRemoteErr('remote URL 不能为空'); return }
    setRemoteSaving(true); setRemoteErr(null)
    try {
      await api.setSyncRemote({ repoPath, remoteUrl: remoteUrl.trim() })
      setRemoteLoaded(true)
    } catch (e) { setRemoteErr(e instanceof Error ? e.message : String(e)) }
  ...
  return (
    <div>
      ...
      {remoteLoaded && !hasRemote && (
        <div style={{ marginTop: 14, padding: 18, border: '1px solid var(--border)', borderRadius: 6, background: 'var(--card)' }}>
          ...
          {remoteErr && (
            <div style={{ marginTop: 8, fontFamily: "'Fira Code', monospace", fontSize: 12, color: 'var(--error)' }}>{remoteErr}</div>
          )}
        </div>
      )}
      ...
      {remoteLoaded && hasRemote && (
        <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
          ...
          <span style={{ fontFamily: "'Fira Code', monospace", color: 'var(--muted)' }}>{remoteUrl}</span>
        </div>
      )}
      ...
      <div className="syncbar" style={{ marginTop: hasRemote ? 12 : 16 }}>
        ...
        <span className="acts">
          <button className="sbtn" onClick={() => run('pull', () => api.syncPull(repoPath), setPull)} disabled={busy !== null || !hasRemote}>
            {busy === 'pull' ? '拉取中…' : '⇅ 拉取'}
          </button>
          <button className="sbtn" onClick={() => run('push', () => api.syncPush(repoPath), setPush)} disabled={busy !== null || !hasRemote}>
            {busy === 'push' ? '上传中…' : '↑ 上传'}
          </button>
        </span>
      </div>
      ...
      {!hasRemote && (
        <div style={{ marginTop: 8, fontSize: 11, color: 'var(--muted)', fontFamily: "'Fira Code', monospace" }}>
          请先配置 remote URL 以启用同步功能
        </div>
      )}

      {error && (
        <div style={{ marginTop: 12, padding: 10, border: '1px solid var(--error)', borderRadius: 6, background: 'var(--card)', fontFamily: "'Fira Code', monospace", fontSize: 13, color: 'var(--error)' }}>
          {error}
        </div>
      )}
