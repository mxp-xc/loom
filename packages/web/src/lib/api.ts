...
export const api = {
  ...
  addMcpServer: (body: { repoPath: string; server: { id: string; type: string; command?: string; args?: string[]; url?: string; headers?: Record<string, string>; env?: Record<string, string>; targets?: string[] } }) =>
  ...
  getSyncRemote: (repoPath: string) =>
    fetch(`${base}/sync/remote?repoPath=${encodeURIComponent(repoPath)}`).then(json) as Promise<{ remoteUrl: string | null }>,
