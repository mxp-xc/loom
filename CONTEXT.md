...
_Avoid_: client application, projection destination

**Registered agent**:
Loom 已知如何集成的 Agent，与当前仓库是否选择它无关。
_Avoid_: configured agent, visible agent

**Configured agent**:
当前仓库 effective `config.agents` 中的 Agent。它限定功能页面可见的 agent，以及 agent-specific projection 和 preview 的候选范围；空集合表示当前仓库不使用任何 agent。
_Avoid_: default agent, installed agent

**Applicable agent**:
支持当前 capability 的 Configured agent。
_Avoid_: supported agent, available agent
...
**Preview agent**:
只读的 agent 视角，用于预览某项配置在 CC/CX/OC 下的变量解析与写入形态；不改变 desired agents，也不触发 projection。
_Avoid_: active agent, projection context
