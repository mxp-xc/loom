...
describe('ClaudeCodeAdapter', () => {
  it('readMcp parses mcpServers from .claude.json', async () => {
    ...
    await fs.writeFile(join(home, '.claude.json'), JSON.stringify({ mcpServers: { playwright: { type: 'stdio', command: 'npx' } } }))
  ...
  it('writeMcp preserves other top-level keys in .claude.json', async () => {
    const fs = new NodeFileSystem()
    await fs.writeFile(join(home, '.claude.json'), JSON.stringify({ otherKey: 'keep', mcpServers: { old: { type: 'stdio', command: 'old' } } }))
    const adapter = new ClaudeCodeAdapter()
