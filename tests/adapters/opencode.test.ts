...
describe('OpenCodeAdapter', () => {
  it('readMcp parses mcp from opencode.json', async () => {
    ...
    await fs.writeFile(join(home, 'opencode.json'), JSON.stringify({ mcp: { playwright: { type: 'stdio', command: 'npx' } } }))
  ...
  it('writeMcp preserves other top-level keys in opencode.json', async () => {
    const fs = new NodeFileSystem()
    await fs.writeFile(join(home, 'opencode.json'), JSON.stringify({ otherKey: 'keep', mcp: { old: { type: 'stdio', command: 'old' } } }))
    const adapter = new OpenCodeAdapter()
