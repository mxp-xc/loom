...
Projection 把 manifest desired state 对齐到 destination-native 文件和配置。本文件定义 projection 可以创建、替换或删除什么。
...
Rule:
Projection 把 Loom 中选择的 desired state 写入合法 destination。MCP 与 Memory destination 是同时满足 Configured 和 Applicable 的 agents；Skills 还支持独立于 Agent Catalog 和 `config.agents` 的 Shared skills destination。Agent 可执行命令是否位于 PATH 不属于 projection 范围。Projection 不能从现有 destination 文件反向推断新的 desired state。

Implications:

- 没有任何 selected destination 的 Skills item 不应该继续保持 projected；`agents: []` 且 `shared: true` 是合法的 shared-only desired state。
- manifest 中选择了 agent 且源内容可用时，应投影到该 agent。
- manifest 中 Skills item 选择 `shared: true` 且源内容可用时，应投影到 `~/.agents/skills`，无需 configured 或 installed Agent。
...
- Skills desired-state mutation 可以携带明确 item 与 destination；targeted projection 只 reconcile这些目标，不把 partial plan 当成全局 desired state。
...
Rule:
当 local skills 使用 copy projection 时，Loom 会在 copied skill 目录写入 .loom-projection.json marker。Cleanup 使用该 marker 区分 managed copy 和 user-owned directory。该契约同时适用于 Agent Skills roots 与 Shared skills root；Remote source namespace 使用 R-PROJECTION-006 的整体 ownership contract。
...
- External local skill 的 canonical source 与 destination 是同一个真实目录时属于 user-owned in-place satisfaction：不创建 link/copy，不写 marker或managed ledger，关闭assignment也不删除该目录。
...
Rule:
Projection 删除 managed 或 linked skill child 后，可以删除随之变空的 parent namespace directories。如果 Skills root 因本次 cleanup 变空，也可以删除该 Skills root，但不能继续删除 Skills root 的父目录。
...
- Shared cleanup 最多删除到 `~/.agents/skills`，不得删除 `~/.agents`。
...
Rule:
每个 remote source 投影到 `<skills-destination>/<source-name>/` 独立 namespace。Projection planner 对每个 destination 收集该 source 的 selected bundle roots 和 selected resource roots，移除它们的最长共同父路径前缀后保留其余目录结构；executor 将该 namespace 作为一个 managed unit 整体替换。
...
- 不同 destination 的 members 可以不同，因此各自计算 projection base；source-global resources 只投影到至少选择了该 source 一个 member 的 destination。
...
- 同一 destination 中，local skill destination 不得与 source namespace 重合或位于其下；planner 必须在任何文件系统写入前拒绝冲突。不同 destination 之间的同名路径互不构成 collision。
- 替换或删除现有 namespace 前必须验证 marker ownership；没有 marker、marker 不匹配或 destination collision 时 projection 失败并保留原内容。
- Namespace staging、替换和跨 destination 执行必须可回滚；全部成功前不能删除 backups 或报告成功。
...
- Orphan cleanup 只枚举 Skills root 的 direct children，不递归进入 namespace，也不跟随 child links。
- Targeted source cleanup 只检查明确的 `source + destination` namespace；不枚举或清理无关 source namespace。
...
## R-PROJECTION-007 Destination 与 source identity 必须在 apply 时仍有效

Status: active
Applies to: skills, MCP, memory projection

Rule:
Projection在首次destination write前验证全部source与destination roots、ancestors和leaf；apply时重新绑定preflight所授权的physical identity。只证明同一路径当前仍存在，不等于仍是已授权对象。
...
- Shared skills root从canonical home trust root解析`.agents/skills`，`.agents`、`skills`及其descendants使用相同的no-follow与identity revalidation；不存在时只按安全directory chain逐级创建。
...
## R-PROJECTION-009 Skills managed state 按 destination 记录

Status: active
Applies to: skills projection state, cleanup, rollback

Rule:
`projected-skills.json` 按稳定 destination key记录Loom-managed local skill artifacts。合法key是`agent:<AgentId>`或`shared`；source namespace ownership继续由namespace marker证明。

Implications:

- v2 state使用destination-keyed结构；读取agent-keyed v1时只接受Registered agent ids并在内存中转换，下一次成功写入使用v2。
- State读取不扫描文件系统补全缺失记录，也不把artifact存在反推为desired state。
- In-place satisfied的user-owned external skill不进入managed state。

Safety:

- Version、owner、destination key或artifact entry malformed，以及state read failure，都记录完整`{ err }`并fail closed，不授权cleanup。
- Cleanup和rollback只操作state或marker能证明归属且physical identity仍匹配的artifact。
