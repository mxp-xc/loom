...
MCP 规则定义 Loom 中 MCP server 的 desired state、agent 应用、配置解析上下文与投影边界。

## R-MCP-001 MCP server 定义与 agent 应用分离

Status: active
Applies to: MCP manifest, MCP UI, MCP API

Rule:
MCP server 的定义保存只更新 desired server definition；agent 应用只更新该 server 选择投影到哪些 agent。新增 MCP server 默认不应用到任何 agent。

Implications:

- Create MCP server 时默认 `agents: []`，不因当前配置视图或当前选中 agent 自动添加 agent。
- Edit MCP server 时保留原有 `agents`，除非用户在 agent 控制中显式修改。
- MCP server row 与全局 agent chip 只更新 desired agent state，不触发 projection。

Safety:

- 不把当前配置视图对应的 agent 误写入 manifest agents。
- 不因保存 server 定义而静默修改 agent 应用范围。

Examples:

- 用户新增 `playwright` server 后，该 server 出现在列表中，但 Applicable MCP agent chips 都是未应用状态。
- 用户编辑 `playwright` 的 env 后，原来已应用到 Codex 的 agent 仍保留。
...
Rule:
MCP 的 agent state 改动不会自动 project。用户必须通过 MCP 页面中的 Project changes 显式触发 MCP projection。

Implications:

- Row agent chip 和全局 agent chip 保存 desired state 后只刷新页面状态。
- MCP 页面 Project changes 按钮调用 MCP scope projection。
- UI 必须把 server 定义、agent 应用、Project changes 表达为三件事。

Safety:

- 不在 agent toggle 成功后报告 projection 已完成。
- 批量 agent 更新中途失败时刷新 manifest，让 UI 显示已保存状态。
...
- Detail 和 Editor header 提供 RAW、Default 与 Applicable MCP agents 的配置视图。
- RAW 保留 `${...}`；Default 只按 Base → Local 解析，不应用 agent override 或 builtin runtime。
- Editor 下方 preview card 使用当前 draft 和配置视图实时展示对应形态。
- Agent preview 的 native path、root key、language 和序列化格式从 Agent Catalog 与共享 codec 派生。
- `${var}` 在 Default 中使用中性结果，在 agent 视图中使用对应 per-agent vars 结果。
- Tools 从 RAW 进入时自动切换到 Default；Default 和 agent 视图都可以创建真实调试连接。

Safety:

- 配置视图切换不能触发保存、agent 应用或 projection。
- 缺失变量、默认值、JSON 插值等诊断必须在 preview 中可见。

Examples:

- 同一个 `${browsers_path}` 在 Claude Code 与 Codex 下可显示不同 resolved value，但 server 的 agents 不变化。
...
Rule:
从 Applicable MCP agents 的原生配置导入 MCP server 必须由用户显式触发并确认。导入只写入 Loom 的 `mcp.yaml` desired state，不修改 agent-native 配置，也不自动运行 projection。

Implications:

- 导入预览展示来源 agent、最终 id、agents、改名和 ignored fields。
- 导入后 agents 按来源生成；同一 server 从多个来源导入时合并 agents。
- 同名同定义只合并 agents；同名不同定义自动改名，保留现有 desired entry。
- 导入成功后仍由用户点击 Project changes 才投影到 agent-native 文件。
- 请求省略 `sources` 时，Server 使用 effective Applicable MCP agents；显式 `sources: []` 表示扫描零来源，不回退到全部 agents。
- Web 始终显式传递当前 Applicable MCP sources；空集合显示稳定空态且不读取原生配置。

Safety:

- 不从磁盘现状静默推断 desired state；只有确认导入的条目会写入 `mcp.yaml`。
- 不覆盖现有不同定义的 desired entry。
- 不修改任何 agent-native MCP 配置文件。

Examples:

- 从 Codex 导入 browser 后，`mcp.yaml` 中该 server 带 `agents: ['codex']`。
...
- Reorder 不修改 server 定义或 agents，也不触发 projection。
