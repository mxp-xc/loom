...
可用的原生 `button` 和 `role="button"` 控件统一使用 `pointer`；拖拽激活器及拖拽过程也保持 `pointer`。`Button` 的 disabled 状态统一使用：
...
主题使用 `light` / `dark` / `auto` / `system` 四态，首次访问默认 `light`。

- `auto` 按本地时间切换：`06:00–17:59` 使用浅色，其余时间使用深色
- `system` 跟随 `prefers-color-scheme`
- no-flash 内联脚本在 `packages/web/index.html` 中读取 `localStorage` key `loom-theme`
- ThemeSwitcher 使用四个带 accessible name 和 pressed state 的图标按钮
