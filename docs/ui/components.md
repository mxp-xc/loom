...
`AgentChip` 用于 Agent Catalog 中的 registered agents。Agent chip 使用 26px 圆形轮廓和品牌 mask icon；完整名称来自 `aria-label`/tooltip，不能只靠颜色或缩写识别。`default` 等非 agent scope 使用对应页面的显式 scope control，不借用真实 agent。
