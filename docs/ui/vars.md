...
`default` 与 Settings 中的 Configured agents 共享同一组查看范围 controls：

- `default` 使用 Base -> Local，不应用 agent override 或 builtin runtime。
- Agent 范围使用 Base -> Base/agent -> Local -> Local/agent -> Runtime。
- 初始范围为 `default`；Configured agents 为空时不制造真实 agent fallback。
- 查看范围只改变表格、预览和 trace，不写入 agents，也不触发 projection。
...
Vars API 位于 `/api/vars`，通过 `repoPath` 绑定授权仓库。页面读取 default 与每个 Configured agent 的 matrix，并通过 base key、override 和 clear 操作更新数据；成功 mutation 后重新读取 matrix，不在客户端猜测最终覆盖结果。
