...
signature 元素:投影连接线。在 Sources 列表底部展示 `manifest ──▶ agent` 的发光节点链,把"一份配置编织到多个 agent"的核心概念直接可视化,这是其他配置管理工具没有的记忆点。
...
- **A(推荐)** Tailwind CSS v4 + shadcn/ui:原子类做布局和 token,shadcn/ui 基于 Radix Primitives 提供无障碍交互组件,代码复制进项目可完全定制。三态主题用 `data-theme` 属性切换。
- **B** Tailwind CSS v4 + 完全自建组件:最大自由度,但 Dialog/Popover/Combobox 从零写工作量大。
- **C** CSS Modules + Radix Primitives:隔离性好,但三态主题需手动维护多套变量映射。
