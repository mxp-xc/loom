...
describe('NodeFileSystem', () => {
  ...
  it.skipIf(platform() !== 'win32')(
    'Windows junction: removeLink 不递归删目标真实内容',
    async () => {
...
async function makeBareWithCommit(): Promise<string> {
  ...
  await wg.pushTags('origin') // 打 tag,覆盖 lsRemote 的 tags 解析分支
  return bare
}
...
export class NodeGit implements IGit {
  ...
  } // path 可选:lsRemote/clone 零参用 cwd

  async init(repoPath: string): Promise<void> {
    await this.git(repoPath).raw(['init', '-b', 'main'])
  }
  async fetch(repoPath: string): Promise<void> {
    await this.git(repoPath).fetch(['--tags'])
  }
  ...
  async lsRemote(url: string): Promise<{ tags: Record<string, string>; head: string }> {
    ...
    for (const line of out.split('\n').filter(Boolean)) {
      ...
      if (ref === 'HEAD') head = sha
        ...
        tags[ref.slice('refs/tags/'.length).replace(/\^\{\}$/, '')] = sha
    }
    return { tags, head }
  }
  ...
  for (const k of allKeys) {
    ...
    if (o && t) {
      ...
      for (const f of new Set([...Object.keys(o ?? {}), ...Object.keys(t ?? {})])) {
        ...
        if (ov === tv) {
          ;(merged as any)[f] = ov
        } else if (bv !== undefined && ov === bv) {
          ;(merged as any)[f] = tv
        } // ours 未改 -> 取 theirs
        else if (bv !== undefined && tv === bv) {
          ;(merged as any)[f] = ov
        } // theirs 未改 -> 取 ours
        else {
...
function mergeObj(
  ...
  for (const k of new Set([...Object.keys(ours), ...Object.keys(theirs)])) {
    ...
    if (o === t || (b !== undefined && o === b)) out[k] = t === undefined ? o : t
      ...
      out[k] = mergeObj(b, o, t, file, path, conflicts) // 嵌套对象递归
